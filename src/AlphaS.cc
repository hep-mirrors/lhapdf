...
namespace LHAPDF {
  ...
  /// @todo Put into AlphaS interface along with a flag to use or not use mass thresholds
  ...
  std::vector<double> AlphaS::betas(int nf) {
    /// @todo Cache these
    std::vector<double> v;
  ...
  double AlphaS_ODE::alphaS_Q2(double q2) {
    ...
    while (fabs(q2 - t)/q2 > accuracy) {
      ...
      f1 = _derivative( t, y, beta );
      k1 = h*f1;

      // Increments based on the slope at the midpoint of the interval
      f2 = _derivative( t + (h/2.0), y + (k1/2.0), beta );
      k2 = h*f2;

      f3 = _derivative( t + (h/2.0), y + (k2/2.0), beta );
      k3 = h*f3;

      // Increment based on the slope at the end of the interval
      f4 = _derivative( t + h, y + k3, beta );
      k4 = h*f4;
