...
public:
    ...
    /// \return 
    static PDFGrid* load( const std::string& );
    ...
    /// \return 
    static PDFGrid* loadByName( const std::string& );
...
//Grid Specific Exceptions

/// UndefinedInterpolatorException is thrown when the PDFGrid is attempting to interpolate without a valid Interpolator.
class UndefinedInterpolatorException: public std::exception {
public:
    UndefinedInterpolatorException( const PDFGrid* PDF )
    : pdf(PDF) {}
    
    const char* what() const throw() {
        std::stringstream message;
        message << "Tried to interpolate without setting the interpolator for PDFGrid* " << pdf << ".";
...
/// UndefinedExtrapolatorException is thrown when the PDFGrid is attempting to extrapolate without a valid Extrapolator.
class UndefinedExtrapolatorException: public std::exception {
public:
    UndefinedExtrapolatorException( const PDFGrid* PDF )
    : pdf(PDF) {}
    
    const char* what() const throw() {
        std::stringstream message;
        message << "Tried to extrapolate without setting the extrapolator for PDFGrid* " << pdf << ".";
        return message.str().c_str();
    }
...
/// NonPhysicalRangeException is thrown when the (X,Q2) coordinate given is not physically valid.
class NonPhysicalRangeException: public std::exception {
public:
    NonPhysicalRangeException( const PDFGrid* PDF, const X_t X, const Q_t Q )
    : pdf(PDF), x(X), q(Q) {}
    
    const char* what() const throw() {
        std::stringstream message;
        message << "Tried to access non physical range: " << x << " " << q << " in PDFGrid* " << pdf << ".";
        return message.str().c_str();
    }
